...
class easyCollection
{
    ...
    public function __construct($modx, $params = null)
    {
        ...
        if (file_exists(MODX_BASE_PATH . "assets/plugins/easyCollection/configs/config.inc.php")) include(MODX_BASE_PATH . "assets/plugins/easyCollection/configs/config.inc.php");
        else {
			$config = array();
            $text = "<?php" . PHP_EOL . '$config=' . var_export($config, 1) . ';';

            $f = fopen(MODX_BASE_PATH . "assets/plugins/easyCollection/configs/config.inc.php", 'w');
            fwrite($f, $text);
            fclose($f);
        }
     ...
     * @param $id
     */
    function setCurrentConfig($key, $id, $file = '')
    {
		if ($file){
			include(MODX_BASE_PATH . "assets/plugins/easyCollection/configs/".$file.".inc.php");
			$this->config = $config;
		}
